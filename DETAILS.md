...
**キャッシュ詳細**:
- **CacheService**: GAS側で問題データをキャッシュ（キー例: `q_ジャンル1_初級`, `a_ジャンル1_初級`, `h_ジャンル1_初級`）
- **キャッシュキー形式**: `[種類]_[ジャンル名]_[レベル名]`
- **キャッシュ期間**: 7日間（604800秒）
...
**キャッシュ詳細**:
- **CacheService**: 正解データ（キー例: `a_ジャンル1_初級`）とヒント情報（キー例: `h_ジャンル1_初級`）をキャッシュから読込
- **キャッシュ期間**: 7日間
...
2. **メタデータの保存**（`saveCertificateMetadata()`）
   ...
   - 保存内容: `{nickname, date, timestamp}` のJSONオブジェクトをBase64エンコードして保存
     - `nickname`: ニックネーム（文字列、例: "山田太郎"）
     - `date`: 合格日（文字列、例: "2025年12月25日"）
     - `timestamp`: タイムスタンプ（数値、例: 1735099200000）
...
**キャッシュ詳細**:
- **CacheService**: 全レベルの問題データをキャッシュから読込（キー例: `q_ジャンル1_初級`, `a_ジャンル1_初級`）
...
### CacheService（GAS側）
| キャッシュキー例 | 保存内容 | 有効期間 | 生成タイミング |
|--------------|---------|---------|---------------|
| `q_ジャンル1_初級` | 問題データ（正解なし） | 7日間（604800秒） | キャッシュ未存在時に自動生成 or 手動リロード時 |
| `a_ジャンル1_初級` | 正解データ | 7日間（604800秒） | キャッシュ未存在時に自動生成 or 手動リロード時 |
| `h_ジャンル1_初級` | ヒント情報 | 7日間（604800秒） | キャッシュ未存在時に自動生成 or 手動リロード時 |
...
| キー | 保存内容 | 削除タイミング |
|-----|---------|--------------|
| `quiz_nickname` | ニックネーム（文字列） | リセット時 |
| `quiz_browser_id` | ブラウザ識別ID（UUID） | リセット時 |
| `ジャンル番号-級番号` (例: `1-1`) | 合格証メタデータ `{nickname: "山田太郎", date: "2025年12月25日", timestamp: 1735099200000}` をBase64エンコードした文字列 | リセット時 |
| `ALL` | エクストラステージ合格証メタデータ（上記と同じ形式） | リセット時 |
...
- **処理内容**: 全ジャンル・全レベルのキャッシュ（問題データ `q_`、正解データ `a_`、ヒント情報 `h_`）をクリアして再構築
